...
Start by updating your [catalog.yaml](./Specifications/catalog.yaml) to list the data domains of your system. These will align with MongoDB Collections in the generated code.This is an example used by the Velocity Chain testing project.
```yaml
data_dictionaries:
  - name: Identity
...
```sh
make install
cp <your token> ~/.flux/GITHUB_TOKEN
make update
source ~/.zshrc
```
...
The ``launch-all`` command may take a while to run, more than 10 minutes. After using the ``fx up all`` command visit the [welcome page](localhost:8080) in your browser.
...
## Big Idea
AI-powered supply chain optimization platform that provides real-time visibility, predictive analytics, and intelligent automation to maximize efficiency and minimize costs across global supply networks.
